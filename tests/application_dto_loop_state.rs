...
#[test]
fn test_dto_status_label_idle() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let engine = LoopEngine::new(clock.clone(), audio_bus);
    let dto = LoopStateDto::from(&engine);

    assert_eq!(dto.status_label(), "idle");
}

#[test]
fn test_dto_status_label_ready() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.status_label(), "ready");
}

#[test]
fn test_dto_status_label_recording() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.status_label(), "recording");
}

#[test]
fn test_dto_status_label_playing() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.status_label(), "playing");
}

#[test]
fn test_dto_status_label_paused() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.status_label(), "PAUSED");
}

#[test]
fn test_dto_display_string_idle() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let engine = LoopEngine::new(clock.clone(), audio_bus);
    let dto = LoopStateDto::from(&engine);

    assert_eq!(dto.display_string(), "Loop idle");
}

#[test]
fn test_dto_display_string_ready() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.display_string(), "Loop ready");
}

#[test]
fn test_dto_display_string_recording() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.display_string(), "Loop recording");
}

#[test]
fn test_dto_display_string_playing_single_track() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.display_string(), "Loop playing (1 track)");
}

#[test]
fn test_dto_display_string_playing_multiple_tracks() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
...
#[test]
fn test_dto_display_string_paused_single_track() {
    let clock = FakeClock::new(500);
    let audio_bus = AudioBusMock::new();
    let mut engine = LoopEngine::new(clock.clone(), audio_bus);
    engine.handle_space(TEST_BPM, TEST_BARS);
    ...
    assert_eq!(dto.display_string(), "Loop paused (1 track)");
}
